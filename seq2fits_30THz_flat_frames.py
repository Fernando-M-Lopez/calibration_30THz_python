...
Running the script: 
 ...
 python seq2fits_30THz_flat_frames path_to_seq_files telescope outdir
 ...
 Return: 
 - The code extract the .fits for every frame of .seq files containing images to create flat field image (method KLL Khun et al. 1991)
 - Using outdir as base the script will create (if not existing) the following directories: outdir + /telescope/yyyymmdd/flats/
...
class Telescope:
    def __init__(self, name):
        ...
        if name == 'AR30T':
            ...
            self.elevation = 2430
            self.comments = ['uncalibrated frame to get flat field calibration image bia KLL method.',
            		      'COPYRIGHT. Grant of use.',
            ...
            self.elevation = 760
            self.comments = ['uncalibrated frame to be used to get flat field calibration image bia KLL method',
            			'COPYRIGHT. Grant of use.',
...
def create_fits_header(frameInfo, imageRoi, objPar, width, height, in_file, telescope, fits_filename=None):
   
    """ Returns:
        fits.Header: The created FITS header.
    """
    ...
    hdr['LVL_NUM']  = ('0.0', ' ')
    hdr['EXPTYPE']  = ('flat_frame', 'frame to build a flat field (method KLL)')
...
def define_fits_filename(header, base_dir):
    ...
    hhmmss_sss = hhmmss_sss.replace('.', '_')

    # Define the directory structure
    output_dir = os.path.join(base_dir, telescope, yyyymmdd, "flat_frames")
    os.makedirs(output_dir, exist_ok=True)

    # Construct the filename
    fits_filename = os.path.join(output_dir, f"{telescope}_{yyyymmdd}T{hhmmss_sss}_flat_frame.fits")

    return fits_filename

#---------------------------------------------------------------------------------
# Main code to read .seq file and create .fits file
...
def process_seq_files(seq_directory, telescope):
    ...
    for seq_file in seq_files:
        ...
        while frame_number is not None:
            if im.get_frame(frame_number):
                ...
                hdr = create_fits_header(frameInfo, imageRoi, objPar, width, height, seq_file, telescope)
